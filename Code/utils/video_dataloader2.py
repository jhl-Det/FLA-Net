...
class SalObjDataset(data.Dataset):
    def __init__(self, root, split_file, trainsize, clip_len=4):
    ...
    def __getitem__(self, index):
        images = [self.binary_loader(x) for x in self.images[index]]
        ...
        images = [self.img_transform(x) for x in images_aug]
...
class test_dataset:
    def __init__(self, root, val_file, testsize, clip_len=4):
    ...
    def load_data(self):
        images = [self.binary_loader(x) for x in self.images[self.index]]
        images = [self.transform(x) for x in images]
        gt = self.binary_loader(self.gts[self.index][0])
        name = self.images[self.index][0]
        image_for_post=self.binary_loader(self.images[self.index][0])
        image_for_post=image_for_post.resize(gt.size)
        if name.endswith('.jpg'):
            name = name.split('.jpg')[0] + '.png'
        self.index += 1
        self.index = self.index % self.size
        return torch.stack(images).unsqueeze(0), gt, name, np.array(image_for_post)
