...
from Code.utils.video_dataloader2 import get_loader,test_dataset
    ...
    try:
        for i, (images, gts) in enumerate(train_loader, start=1):
            for rate in size_rates:
                ...
                # print(images.shape)
                if rate != 1:
                    frm1 = F.interpolate(images[:,0,...], size=(trainsize, trainsize), mode='bilinear', align_corners=True)
                    frm2 = F.interpolate(images[:,1,...], size=(trainsize, trainsize), mode='bilinear', align_corners=True)
                    frm3 = F.interpolate(images[:,2,...], size=(trainsize, trainsize), mode='bilinear', align_corners=True)
                    images = torch.stack([frm1, frm2, frm3], dim=1)
